...
export default function StickyPanel({ lang = 'ru' }: { lang?: Lang }) {
    ...
    >
      <div className="bg-white/95 backdrop-blur-md border-t border-gray-200 shadow-[0_-4px_20px_rgba(0,0,0,0.08)]" style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-center gap-3">
          ...
          <a
            href="#process"
            className="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 bg-gradient-to-r from-gold to-gold-light text-ocean-deep font-semibold rounded-full px-5 py-2.5 text-sm transition-all shadow-sm gold-glow"
          >
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M6 12L3.269 3.126A59.768 59.768 0 0121.485 12 59.77 59.77 0 013.27 20.876L5.999 12zm0 0h7.5" /></svg>
            <span className="hidden sm:inline">{lang === 'de' ? 'Reise planen' : 'Подобрать путешествие'}</span>
            <span className="sm:hidden">{lang === 'de' ? 'Planen' : 'Подобрать'}</span>
          </a>
        </div>
      </div>
    </div>
